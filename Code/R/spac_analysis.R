# soil-plant-atmosphere continuum analysis
# analyze leaf water potential data in relation to
# in relation to soil water potential, sap flow, and TWD
...
LWP_ins = read_csv("../../Data/Pfyn/Insitu_lwp.csv")
LWP_ins$predawn = LWP_ins$predawn / -10
LWP_ins$midday = LWP_ins$midday / -10
LWP_ins = LWP_ins %>% group_by(date, treatment) %>% summarize_at(vars(predawn, midday), list(mean=mean, sd=sd))
LWP_ins$meta = if_else(LWP_ins$treatment == "stop irrigation", "stop", LWP_ins$treatment)
LWP_vpd = rbind(select(LWP_ins, -c(treatment)), LWP24)
LWP_vpd = rbind(LWP_vpd, LWP25)
LWP = filter(LWP_vpd, meta %in% c("control", "stop", "irrigation"))
...
sap = sap %>% left_join(sap_max) %>% mutate(Sap_Flow_norm = Total_Sap_Flow / max_sap) %>% 
  select(-max_sap)

# exclude winter months
TWD$month = month(TWD$date)
TWD[TWD$month < 5 | TWD$month > 11, c("twd_pd", "twd_pdn", "twd_md", "MDS", "MDS_norm")] = NA
...
# combine leaf water potential with TWD for regressions
...
ggplot(filter(SWP_pd, date >= "2022-08-01", date<"2025-10-01"), aes(date, SWP/1000, color=as.factor(depth), fill=as.factor(depth)))+
  stat_summary(geom="line", fun=mean)+stat_summary(geom="ribbon", alpha=0.5)+
  geom_point(data=filter(TWD_lwp, date>="2022-08-01", date<"2025-10-01"), aes(date, LWP_pd), color="black", inherit.aes=F)+
...
ggplot(filter(SWP_pd, meta=="control", date >= "2022-08-01", date<"2025-10-01"), aes(date, SWP/1000, color="SWP"))+
  stat_summary(geom="line", fun=mean)+stat_summary(geom="ribbon", alpha=0.5)+
  geom_point(data=filter(TWD_lwp, meta=="control", date>="2022-08-01", date<"2025-10-01"), aes(date, LWP_pd, color="TWD-derived LWP"))+
  geom_pointrange(data=filter(LWP, meta=="control", date<"2025-10-01"), aes(x=date, y=predawn_mean, ymin=predawn_mean-predawn_sd, ymax=predawn_mean+predawn_sd, color="Measured LWP"), inherit.aes=F)+
  theme_bw()+labs(x="", y="Pre-dawn SWP / LWP (MPa)")+scale_colour_manual(name="Data", values=cols, breaks=c("Measured LWP", "TWD-derived LWP", "SWP"))+
  theme(legend.position="bottom", legend.text=element_text(size=12), legend.title=element_text(size=14),
        plot.title=element_text(hjust=0.5), axis.title.y=element_text(size=14), axis.text=element_text(size=12))+
  ggtitle("Comparison of measured, temperature-corrected pre-dawn Soil Water Potential (SWP)\nagainst measured and Tree Water Deficit (TWD)-derived pre-dawn Leaf Water Potential (LWP)\nin Pfynwald Control scenario")
...
p_wp = ggplot(filter(SWP_pd, meta=="control", date>="2024-05-01", date<"2025-10-01"), aes(date, SWP/1000, color=as.factor(depth), fill=as.factor(depth)))+
  stat_summary(geom="line", fun=mean)+stat_summary(geom="ribbon", alpha=0.5)+
  geom_pointrange(data=filter(LWP, meta=="control", date>="2024-05-01", date<"2025-10-01"), aes(x=date, y=predawn_mean, ymin=predawn_mean-predawn_sd, ymax=predawn_mean+predawn_sd), color="green", inherit.aes=F)+
  theme_bw()+labs(x="", y="Pre-dawn SWP / LWP (MPa)", fill="Depth")+theme(legend.position="inside", legend.position.inside = c(.5,.5))+guides(color="none")
...
# only sap flow and water potentials
p_sap = ggplot(filter(sap, Treatment=="control", date>="2024-05-01", date<"2025-11-01"), aes(date, Total_Sap_Flow))+geom_line(color="darkgreen", linewidth=0.8)+
  theme_bw()+labs(x="", y="Daily Sap Flow (kg/h)")+theme(axis.title.y=element_text(size=14), axis.text=element_text(size=12))
cols = c("SWP"="black","Measured LWP"="red", "TWD-derived LWP"="blue")
p_wp = ggplot(filter(SWP_pd, meta=="control", date >= "2024-05-01", date<"2025-11-01"), aes(date, SWP/1000, color="SWP"))+
  stat_summary(geom="line", fun=mean)+stat_summary(geom="ribbon", alpha=0.5)+
  geom_point(data=filter(TWD_lwp, meta=="control", date>="2024-05-01", date<"2025-11-01"), aes(date, LWP_pd, color="TWD-derived LWP"))+
  geom_pointrange(data=filter(LWP, meta=="control", date>="2024-05-01", date<"2025-11-01"), aes(x=date, y=predawn_mean, ymin=predawn_mean-predawn_sd, ymax=predawn_mean+predawn_sd, color="Measured LWP"), inherit.aes=F)+
  theme_bw()+labs(x="", y="Pre-dawn SWP / LWP (MPa)")+scale_colour_manual(name="Data", values=cols, breaks=c("Measured LWP", "TWD-derived LWP", "SWP"))+
  theme(legend.position="bottom", legend.text=element_text(size=12), legend.title=element_text(size=14),
        plot.title=element_text(hjust=0.5), axis.title.y=element_text(size=14), axis.text=element_text(size=12))

grid.arrange(p_sap, p_wp)
...
# soil water potential by depth
ggplot(filter(SWP_md, date >= "2022-08-01"), aes(date, SWP/1000, color=as.factor(depth), fill=as.factor(depth)))+
  stat_summary(geom="line", fun=mean)+stat_summary(geom="ribbon", alpha=0.5)+
...
# average soil water potential
ggplot(filter(SWP_md, date >= "2022-08-01"), aes(date, SWP/1000))+
  stat_summary(geom="line", fun=mean)+stat_summary(geom="ribbon", alpha=0.5)+
...
# average soil water potential and twd-derived leaf water potential
ggplot(filter(SWP_md, date >= "2022-08-01", date<"2025-10-01"), aes(date, SWP/1000))+
  stat_summary(geom="line", fun=mean)+stat_summary(geom="ribbon", alpha=0.5)+
  geom_point(data=filter(TWD_lwp, date>="2022-08-01", date<"2025-10-01"), aes(date, LWP_md), color="blue")+
  geom_pointrange(data=filter(LWP, date<"2025-10-01"), aes(x=date, y=midday_mean, ymin=midday_mean-midday_sd, ymax=midday_mean+midday_sd), color="red", inherit.aes=F)+
  facet_wrap(~meta, ncol=1)+theme_bw()+labs(x="", y="Midday SWP / LWP (MPa)")+
  theme(axis.title.y=element_text(size=14), axis.text=element_text(size=12), strip.text=element_text(size=12))
...
sap_lwp2 = inner_join(LWP_vpd, rename(sap, meta=Treatment))
