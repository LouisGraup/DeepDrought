...
function behavioral_met(met)
    ...
    return(met[met.twd_pd_cor .< -0.76 .&&
               met.twd_md_cor .< -0.78 .&&
               met.lwp_pd_cor .> 0.66 .&&
               met.lwp_md_cor .> 0.81, :])
...
met_good = behavioral_met(met);
println("$(size(met_good, 1)) behavioral parameter sets out of total $(size(met, 1)).")
describe(met_good)
...
plot(ks_plots..., size=(1200,1200), layout=(2,2), legend=false, titlefontsize=8, guidefontsize=6)
