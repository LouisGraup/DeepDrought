...
param = [
    # hydro parameters
    ("DRAIN", 0.2, 0.9), # drainage (0, 1)
    ("INFEXP", 0.2, 0.9), # infiltration exponent (0, 0.9)
    ("IDEPTH_m", 0.3, 0.9), # infiltration depth (m) (0.05, 1.0)
    # meteo parameters
    #("ALB", 0.15, 0.3), # surface albedo (0.1, 0.3)
    #("ALBSN", 0.4, 0.8), # snow surface albedo (0.4, 0.8)
    # soil parameters
    ("RSSA", 50, 800), # soil resistance (20, 1000)
    ("ths1", 0.47, 0.6), # theta_sat (0.15, 0.6)
    ("thr1", 0.01, 0.07), # theta_res (0.0, 0.1)
    ("ksat1", -0.4, 0.5), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("alpha1", 0.5, 1.12), # multiplier on alpha (0.5, 1.5)
    ("npar1", 1.2, 1.27), # n (1.15, 1.3)
    ("ths2", 0.2, 0.24), # theta_sat (0.15, 0.6)
    ("thr2", 0.01, 0.025), # theta_res (0.0, 0.1)
    ("ksat2", -0.3, 0.05), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("alpha2", 0.7, 1.4), # multiplier on alpha (0.5, 1.5)
    ("npar2", 1.22, 1.28), # n (1.15, 1.3)
    ("ths3", 0.18, 0.25), # theta_sat (0.15, 0.6)
    ("thr3", 0.0, 0.02), # theta_res (0.0, 0.1)
    ("ksat3", -0.3, 0.4), # additive factor on log10(k_sat) (-0.5, 0.5)
    ...
    ("GLMAX", 0.004, 0.009), # stomatal conductance (0.001, 0.02)
    ("CVPD", 0.7, 2.8), # vpd sensitivity (0.5, 3)
    ("R5", 60, 180), # radiation sensitivity (50, 200)
    #("T1", 6, 12), # low temperature threshold (5, 15)
    #("T2", 20, 35), # high temperature threshold (20, 35)
    ("PSICR", -1.5, -1.25), # critical water potential (-3, -1)
    ...
    ("BETAROOT", 0.965, 0.983) # beta root coefficient (0.9, 0.999)
];
