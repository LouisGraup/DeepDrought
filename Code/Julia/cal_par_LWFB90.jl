...
param = [
    ...
    ("INFEXP", 0.1, 0.6), # infiltration exponent (0, 0.9)
    ...
    ("ths1", 0.3, 1.1), # multiplier on theta_sat (0.5, 1.5)
    ...
    ("ths3", 0.3, 0.5), # multiplier on theta_sat (0.5, 1.5)
    ...
    ("FRINTLAI", 0.06, 0.2), # interception catch fraction per unit LAI (0.02, 0.2)
    ("GLMAX", 0.001, 0.015), # stomatal conductance (0.001, 0.03)
    ("CVPD", 1.0, 2.0), # vpd sensitivity (1, 3)
    ("R5", 100, 300), # radiation sensitivity (50, 400)
    #("T1", 6, 12), # low temperature threshold (5, 15)
    #("T2", 20, 35), # high temperature threshold (20, 35)
    ("PSICR", -2.3, -1.2), # critical water potential (-4, -1)
    ("FXYLEM", 0.25, 0.75), # aboveground xylem fraction (0.2, 0.8)
    ("MXKPL", 1.0, 16.0), # maximum plant conductivity (1, 30)
    ("MXRTLN", 1000, 4000), # maximum root length (100, 6000)
    ("VXYLEM_mm", 10.0, 80.0), # xylem volume (5, 80)
    ("DISPERSIVITY_mm", 30.0, 50.0), # dispersivity coefficient (30, 50)
    ("MAXROOTDEPTH", -2.0, -0.8), # max rooting depth (-5, -0.5)
    ("BETAROOT", 0.9, 0.96) # beta root coefficient (0.8, 1.0)
];
