...
param = [
    ...
    ("FRINTLAI", 0.12, 0.18), # interception catch fraction per unit LAI (0.02, 0.2)
    ("GLMAX", 0.0025, 0.004), # stomatal conductance (0.001, 0.02)
    ("CVPD", 0.6, 1.26), # vpd sensitivity (0.5, 3)
    ("R5", 100, 200), # radiation sensitivity (50, 200)
    #("T1", 6, 12), # low temperature threshold (5, 15)
    #("T2", 20, 35), # high temperature threshold (20, 35)
    ("PSICR", -1.25, -1.05), # critical water potential (-3, -1)
    ("FXYLEM", 0.3, 0.5), # aboveground xylem fraction (0.1, 0.5)
    ("MXKPL", 8.0, 18.0), # maximum plant conductivity (7, 30)
    #("MXRTLN", 2000, 4000), # maximum root length (2000, 4000)
    ("VXYLEM_mm", 10.0, 60.0), # xylem volume (5, 80)
    ("DISPERSIVITY_mm", 38, 43), # dispersivity coefficient (30, 50)
    ("MAXROOTDEPTH", -1.8, -1.1), # max rooting depth (-2, -0.8)
    ("BETAROOT", 0.97, 0.98) # beta root coefficient (0.9, 0.999)
];
