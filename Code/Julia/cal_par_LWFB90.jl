...
param = [
    ...
    ("IDEPTH_m", 0.25, 1.0), # infiltration depth (m) (0.05, 1.0)
    # meteo parameters
    #("ALB", 0.15, 0.3), # surface albedo (0.1, 0.3)
    #("ALBSN", 0.4, 0.8), # snow surface albedo (0.4, 0.8)
    # soil parameters
    ("RSSA", 50, 1000), # soil resistance (20, 1000)
    ("ths1", 0.4, 0.6), # theta_sat (0.15, 0.6)
    ("thr1", 0.01, 0.1), # theta_res (0.0, 0.1)
    ("ksat1", -0.4, 0.5), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("alpha1", 0.5, 1.21), # multiplier on alpha (0.5, 1.5)
    ("npar1", 1.19, 127), # n (1.15, 1.3)
    ("ths2", 0.2, 0.25), # theta_sat (0.15, 0.6)
    ("thr2", 0.01, 0.04), # theta_res (0.0, 0.1)
    ("ksat2", -0.5, 0.05), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("alpha2", 0.7, 1.5), # multiplier on alpha (0.5, 1.5)
    ("npar2", 1.22, 1.28), # n (1.15, 1.3)
    ("ths3", 0.17, 0.25), # theta_sat (0.15, 0.6)
    ("thr3", 0.0, 0.03), # theta_res (0.0, 0.1)
    ("ksat3", -0.3, 0.5), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("alpha3", 0.6, 1.4), # multiplier on alpha (0.5, 1.5)
    ("npar3", 1.23, 1.3), # n (1.15, 1.3)
    # plant parameters
    ...
    ("GLMAX", 0.002, 0.015), # stomatal conductance (0.001, 0.02)
    ("CVPD", 0.7, 3.0), # vpd sensitivity (0.5, 3)
    ("R5", 60, 180), # radiation sensitivity (50, 200)
    #("T1", 6, 12), # low temperature threshold (5, 15)
    #("T2", 20, 35), # high temperature threshold (20, 35)
    ("PSICR", -1.5, -1.1), # critical water potential (-3, -1)
    ("FXYLEM", 0.1, 0.5), # aboveground xylem fraction (0.1, 0.5)
    ("MXKPL", 10.0, 28.0), # maximum plant conductivity (7, 30)
    ...
    ("BETAROOT", 0.965, 0.99) # beta root coefficient (0.9, 0.999)
];
