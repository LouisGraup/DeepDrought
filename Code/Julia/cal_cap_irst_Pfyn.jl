...
@everywhere begin
    ...
    obs_twd = obs_twd[obs_twd.scenario .== "irrigation stop", :]; # filter for control scenario
...
@everywhere begin
    ...
    # input
    input_path = "LWFB_testcap/irr_stop/";
    input_prefix = "pfynwald";

    # output
    output_path = "LWFBcalibration/";
    subdir_name = "pfyn_irst_cap";
...
param = [
    # plant parameters
    ("CAPACITANCE", 5, 10.0), # capacitance (0.1, 10)
    ("STORAGEK", 3, 10.0), # storage conductance (0.1, 10)
    ("VSTORAGE", 15.0, 20.0), # stem storage volume (1, 20)
    ("PSICR", -3.0, -2.0) # critical water potential (-3, -1)
];
...
CSV.write("LWFBcal_output/param_pfyn_irst_cap_" * curDate * ".csv", param_out);
...
@everywhere function run_calibration(i)
    ...
    if root_params
        ...
        simulate_isotopes = false, simulate_irrigation = true,
        root_distribution = (beta = 0.965484, z_rootMax_m=-1.63381));
    end
    ...
    try
        ...
        return (par_id, fill(0, 2), fill(0, 4)) # skip if simulation fails
    end
...
CSV.write("LWFBcal_output/metrics_pfyn_irst_cap_" * curDate * ".csv", metrics);
