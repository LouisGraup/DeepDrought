...
param = [
    ...
    # soil parameters
    ("RSSA", 250, 1000), # soil resistance (20, 1000)
    ...
    ("alpha2", 0.5, 1.3), # multiplier on alpha (0.5, 1.5)
    ...
    ("alpha4", 0.5, 1.4), # multiplier on alpha (0.5, 1.5)
    ("npar4", 1.225, 1.3), # n (1.2, 1.3)
    ("ths5", 0.2, 0.4), # theta_sat (0.2, 0.6)
    ("thr5", 0.02, 0.1), # theta_res (0.0, 0.1)
    ("ksat5", 0.25, 0.6), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("alpha5", 0.5, 0.75), # multiplier on alpha (0.5, 1.5)
    ("npar5", 1.25, 1.38), # n (1.25, 1.4)
    # plant parameters
    ("CINTRL", 0.1, 0.7), # interception storage capacity per unit LAI (0.05, 0.75)
    ...
    ("PSICR", -1.9, -1.5), # critical water potential (-4, -1)
