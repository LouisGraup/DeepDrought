...
param = [
    ...
    ("thr4", 0.01, 0.12), # theta_res (0.0, 0.2)
    ("ksat4", -0.4, 0.5), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("alpha4", 0.5, 1.3), # multiplier on alpha (0.5, 1.5)
    ("npar4", 1.2, 1.3), # n (1.2, 1.3)
    ("ths5", 0.2, 0.35), # theta_sat (0.2, 0.6)
    ("thr5", 0.1, 0.2), # theta_res (0.0, 0.2)
    ("ksat5", 0.2, 0.8), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("alpha5", 0.3, 0.6), # multiplier on alpha (0.5, 1.5)
    ("npar5", 1.25, 1.38), # n (1.25, 1.4)
    # plant parameters
    ("CINTRL", 0.3, 0.75), # interception storage capacity per unit LAI (0.05, 0.75)
    ("FRINTLAI", 0.09, 0.2), # interception catch fraction per unit LAI (0.02, 0.2)
    ("GLMAX", 0.001, 0.005), # stomatal conductance (0.001, 0.02)
    ("CVPD", 0.6, 2.0), # vpd sensitivity (0.5, 3)
    ("R5", 50, 200), # radiation sensitivity (50, 200)
    ("T1", 8, 14), # low temperature threshold (5, 15)
    ("T2", 24, 35), # high temperature threshold (20, 35)
    ("PSICR", -2.0, -1.7), # critical water potential (-4, -1)
    ("FXYLEM", 0.1, 0.5), # aboveground xylem fraction (0.1, 0.5)
    ("MXKPL", 10.0, 25.0), # maximum plant conductivity (7, 30)
