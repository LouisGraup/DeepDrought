...
param = [
    ...
    # plant parameters
    ("CINTRL", 0.5, 0.75), # interception storage capacity per unit LAI (0.05, 0.75)
    ("FRINTLAI", 0.09, 0.18), # interception catch fraction per unit LAI (0.02, 0.2)
    ("GLMAX", 0.002, 0.005), # stomatal conductance (0.001, 0.02)
    ("CVPD", 0.6, 1.25), # vpd sensitivity (0.5, 3)
    ("R5", 75, 180), # radiation sensitivity (50, 200)
    ("T1", 9, 14), # low temperature threshold (5, 15)
    ("T2", 25, 34), # high temperature threshold (20, 35)
    ("PSICR", -1.9, -1.7), # critical water potential (-4, -1)
    ("FXYLEM", 0.15, 0.5), # aboveground xylem fraction (0.1, 0.5)
    ("MXKPL", 12.0, 24.0), # maximum plant conductivity (7, 30)
