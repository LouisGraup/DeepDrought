...
param = [
    ...
    # soil parameters
    ("RSSA", 300, 370), # soil resistance (20, 1000)
    ("ths1", 0.2, 0.4), # theta_sat (0.15, 0.6)
    ("thr1", 0.005, 0.05), # theta_res (0.0, 0.1)
    ("ksat1", 0.23, 0.4), # additive factor on log10(k_sat) (-0.5, 0.5)
    ...
    ("alpha3", 1.2, 1.5), # multiplier on alpha (0.5, 1.5)
    ("npar3", 1.2, 1.75), # n (1.15, 1.3)
    # plant parameters
    #("CINTRL", 0.1, 0.75), # interception storage capacity per unit LAI (0.05, 0.75)
    ("FRINTLAI", 0.14, 0.19), # interception catch fraction per unit LAI (0.02, 0.2)
    ("GLMAX", 0.0015, 0.0022), # stomatal conductance (0.001, 0.02)
    ("CVPD", 1.25, 2.0), # vpd sensitivity (0.5, 3)
    ("R5", 75, 150), # radiation sensitivity (50, 200)
