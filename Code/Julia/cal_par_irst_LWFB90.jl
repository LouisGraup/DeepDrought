...
param = [
    # hydro parameters
    ("DRAIN", 0.1, 0.18), # drainage (0, 1)
    ("INFEXP", 0.5, 0.7), # infiltration exponent (0, 0.9)
    ("IDEPTH_m", 0.67, 0.8), # infiltration depth (m) (0.05, 1.0)
    # meteo parameters
    #("ALB", 0.15, 0.3), # surface albedo (0.1, 0.3)
    #("ALBSN", 0.4, 0.8), # snow surface albedo (0.4, 0.8)
    # soil parameters
    ("RSSA", 600, 800), # soil resistance (20, 1000)
    ("ths1", 0.525, 0.6), # theta_sat (0.15, 0.6)
    ("thr1", 0.0, 0.03), # theta_res (0.0, 0.1)
    ("ksat1", -0.2, 0.2), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("alpha1", 0.6, 0.85), # multiplier on alpha (0.5, 1.5)
    ("npar1", 1.18, 1.25), # n (1.15, 1.3)
    ("ths2", 0.195, 0.215), # theta_sat (0.15, 0.6)
    ("thr2", 0.006, 0.014), # theta_res (0.0, 0.1)
    ("ksat2", -0.4, -0.05), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("alpha2", 0.9, 1.2), # multiplier on alpha (0.5, 1.5)
    ("npar2", 1.2, 1.23), # n (1.15, 1.3)
    ("ths3", 0.21, 0.24), # theta_sat (0.15, 0.6)
    ("thr3", 0.0, 0.004), # theta_res (0.0, 0.1)
    ("ksat3", 0.1, 0.35), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("alpha3", 0.65, 0.9), # multiplier on alpha (0.5, 1.5)
    ("npar3", 1.275, 1.3), # n (1.15, 1.3)
    # plant parameters
    #("CINTRL", 0.1, 0.75), # interception storage capacity per unit LAI (0.05, 0.75)
    ("FRINTLAI", 0.18, 0.2), # interception catch fraction per unit LAI (0.02, 0.2)
    ("GLMAX", 0.0055, 0.0066), # stomatal conductance (0.001, 0.02)
    ("CVPD", 0.8, 1.3), # vpd sensitivity (0.5, 3)
    ("R5", 60, 120), # radiation sensitivity (50, 200)
    #("T1", 6, 12), # low temperature threshold (5, 15)
    #("T2", 20, 35), # high temperature threshold (20, 35)
    ("PSICR", -1.6, -1.55), # critical water potential (-3, -1)
    ("FXYLEM", 0.25, 0.33), # aboveground xylem fraction (0.1, 0.5)
    ("MXKPL", 8.0, 11.0), # maximum plant conductivity (7, 30)
    ...
    ("MAXROOTDEPTH", -1.8, -1.4), # max rooting depth (-2, -0.8)
    ("BETAROOT", 0.961, 0.972) # beta root coefficient (0.9, 0.999)
];
