...
param = [
    # hydro parameters
    ("DRAIN", 0.6, 1.0), # drainage (0, 1)
    ("INFEXP", 0.25, 0.8), # infiltration exponent (0, 0.9)
    ("IDEPTH_m", 0.2, 0.4), # infiltration depth (m) (0.05, 1.0)
    # meteo parameters
    #("ALB", 0.15, 0.3), # surface albedo (0.1, 0.3)
    #("ALBSN", 0.4, 0.8), # snow surface albedo (0.4, 0.8)
    # soil parameters
    ("RSSA", 300, 450), # soil resistance (20, 1000)
    ("ths1", 0.2, 0.4), # theta_sat (0.15, 0.6)
    ("thr1", 0.0, 0.05), # theta_res (0.0, 0.1)
    ("ksat1", -0.1, 0.4), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("alpha1", 0.6, 1.1), # multiplier on alpha (0.5, 1.5)
    ("npar1", 1.18, 1.25), # n (1.15, 1.3)
    ("ths2", 0.18, 0.225), # theta_sat (0.15, 0.6)
    ("thr2", 0.0, 0.04), # theta_res (0.0, 0.1)
    ("ksat2", -0.2, 0.2), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("alpha2", 1.2, 1.45), # multiplier on alpha (0.5, 1.5)
    ("npar2", 1.19, 1.23), # n (1.15, 1.3)
    ("ths3", 0.15, 0.18), # theta_sat (0.15, 0.6)
    ("thr3", 0.005, 0.05), # theta_res (0.0, 0.1)
    ("ksat3", -0.4, 0.4), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("alpha3", 1.1, 1.5), # multiplier on alpha (0.5, 1.5)
    ("npar3", 1.2, 1.3), # n (1.15, 1.3)
    # plant parameters
    #("CINTRL", 0.1, 0.75), # interception storage capacity per unit LAI (0.05, 0.75)
    ("FRINTLAI", 0.12, 0.19), # interception catch fraction per unit LAI (0.02, 0.2)
    ("GLMAX", 0.0015, 0.0022), # stomatal conductance (0.001, 0.02)
    ("CVPD", 1.25, 2.2), # vpd sensitivity (0.5, 3)
    ("R5", 60, 175), # radiation sensitivity (50, 200)
    ...
    ("MXKPL", 7.0, 28.0), # maximum plant conductivity (7, 30)
    #("MXRTLN", 2000, 4000), # maximum root length (2000, 4000)
    ("VXYLEM_mm", 25.0, 60.0), # xylem volume (5, 80)
    ("DISPERSIVITY_mm", 35.0, 45.0), # dispersivity coefficient (30, 50)
    ("MAXROOTDEPTH", -1.75, -1.0), # max rooting depth (-2, -0.8)
    ("BETAROOT", 0.965, 0.975) # beta root coefficient (0.9, 0.999)
];
