...
param = [ # store unmodified parameter ranges here for posterity
    ...
    ("npar1", 1.18, 1.29), # n (1.15, 1.3)
    ("ths2", 0.16, 0.23), # theta_sat (0.15, 0.6)
    ("thr2", 0.0, 0.05), # theta_res (0.0, 0.1)
    ("ksat2", -0.5, 0.1), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("alpha2", 0.7, 1.3), # multiplier on alpha (0.5, 1.5)
    ("npar2", 1.16, 1.29), # n (1.15, 1.3)
    ("ths3", 0.16, 0.22), # theta_sat (0.15, 0.6)
    ("thr3", 0.0, 0.04), # theta_res (0.0, 0.1)
    ("ksat3", -0.25, 0.5), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("alpha3", 0.7, 1.5), # multiplier on alpha (0.5, 1.5)
    ("npar3", 1.225, 1.29), # n (1.15, 1.3)
    # plant parameters
    #("CINTRL", 0.1, 0.75), # interception storage capacity per unit LAI (0.05, 0.75)
    ("FRINTLAI", 0.11, 0.18), # interception catch fraction per unit LAI (0.02, 0.2)
    ("GLMAX", 0.006, 0.012), # stomatal conductance (0.001, 0.02)
    ("CVPD", 0.5, 2.5), # vpd sensitivity (0.5, 3)
    ("R5", 60, 200), # radiation sensitivity (50, 200)
    ...
    ("MXKPL", 7.5, 15.0), # maximum plant conductivity (7, 30)
    ...
    ("BETAROOT", 0.91, 0.955) # beta root coefficient (0.9, 0.999)
];
