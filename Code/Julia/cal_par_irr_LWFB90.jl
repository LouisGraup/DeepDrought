...
param = [
    ...
    # soil parameters
    ("RSSA", 1, 1000), # soil resistance (1, 1500)
    ("ths1", 0.4, 1.3), # multiplier on theta_sat (0.5, 1.5)
    ("ksat1", -0.4, 0.5), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("ths2", 0.3, 0.55), # multiplier on theta_sat (0.5, 1.5)
    ("ksat2", -0.5, 0.0), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("ths3", 0.35, 0.45), # multiplier on theta_sat (0.5, 1.5)
    ("ksat3", -0.4, 0.1), # additive factor on log10(k_sat) (-0.5, 0.5)
    # plant parameters
    ...
    ("GLMAX", 0.001, 0.015), # stomatal conductance (0.001, 0.03)
    ("CVPD", 1.1, 2.8), # vpd sensitivity (1, 3)
    ("R5", 150, 380), # radiation sensitivity (50, 400)
    #("T1", 6, 12), # low temperature threshold (5, 15)
    #("T2", 20, 35), # high temperature threshold (20, 35)
    ("PSICR", -2.4, -1.0), # critical water potential (-4, -1)
    ("FXYLEM", 0.2, 0.8), # aboveground xylem fraction (0.2, 0.8)
    ("MXKPL", 1.0, 22.0), # maximum plant conductivity (1, 30)
    ("MXRTLN", 1000, 6000), # maximum root length (100, 6000)
    ("VXYLEM_mm", 5.0, 60.0), # xylem volume (5, 80)
    ("DISPERSIVITY_mm", 30.0, 50.0), # dispersivity coefficient (30, 50)
    ("MAXROOTDEPTH", -2.0, -0.6), # max rooting depth (-5, -0.5)
    ("BETAROOT", 0.9, 0.98) # beta root coefficient (0.8, 1.0)
];
