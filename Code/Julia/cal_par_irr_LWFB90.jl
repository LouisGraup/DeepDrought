...
@everywhere begin
    ...
    obs_soil_iso_irr = select(obs_soil_iso_irr, :date, :depth, :d18O); # remove extra columns
    obs_soil_iso_irr = unstack(obs_soil_iso_irr, :date, :depth, :d18O); # reshape data
    rename!(obs_soil_iso_irr, [:date, :d18O_40cm, :d18O_20cm, :d18O_5cm]); # rename columns

    obs_xy_iso_irr = obs_xy_iso[obs_xy_iso.treatment .== "irrigation", :]; # select irrigation treatment
    select!(obs_xy_iso_irr, :date, :d18O); # remove extra columns
end
...
param = [
    # hydro parameters
    ("DRAIN", 0.0, 0.35), # drainage (0, 1)
    ("INFEXP", 0.75, 0.9), # infiltration exponent (0, 0.9)
    ("IDEPTH_m", 0.9, 1.0), # infiltration depth (m) (0.05, 1.0)
    # meteo parameters
    #("ALB", 0.15, 0.3), # surface albedo (0.1, 0.3)
    #("ALBSN", 0.4, 0.8), # snow surface albedo (0.4, 0.8)
    # soil parameters
    ("RSSA", 300, 550), # soil resistance (20, 1000)
    ("ths1", 0.35, 0.5), # theta_sat (0.15, 0.6)
    ("thr1", 0.01, 0.035), # theta_res (0.0, 0.1)
    ("ksat1", 0.0, 0.5), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("alpha1", 0.5, 1.0), # multiplier on alpha (0.5, 1.5)
    ("npar1", 1.22, 1.29), # n (1.15, 1.3)
    ("ths2", 0.15, 0.18), # theta_sat (0.15, 0.6)
    ("thr2", 0.01, 0.04), # theta_res (0.0, 0.1)
    ("ksat2", -0.5, -0.25), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("alpha2", 1.0, 1.4), # multiplier on alpha (0.5, 1.5)
    ("npar2", 1.17, 1.22), # n (1.15, 1.3)
    ("ths3", 0.15, 0.16), # theta_sat (0.15, 0.6)
    ("thr3", 0.025, 0.055), # theta_res (0.0, 0.1)
    ("ksat3", 0.1, 0.3), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("alpha3", 1.0, 1.4), # multiplier on alpha (0.5, 1.5)
    ("npar3", 1.23, 1.29), # n (1.15, 1.3)
    # plant parameters
    #("CINTRL", 0.1, 0.75), # interception storage capacity per unit LAI (0.05, 0.75)
    ("FRINTLAI", 0.05, 0.2), # interception catch fraction per unit LAI (0.02, 0.2)
    ("GLMAX", 0.002, 0.004), # stomatal conductance (0.001, 0.02)
    ("CVPD", 0.8, 2.0), # vpd sensitivity (0.5, 3)
    ("R5", 90, 200), # radiation sensitivity (50, 200)
    #("T1", 6, 12), # low temperature threshold (5, 15)
    #("T2", 20, 35), # high temperature threshold (20, 35)
    ("PSICR", -1.25, -1.0), # critical water potential (-3, -1)
    ("FXYLEM", 0.15, 0.45), # aboveground xylem fraction (0.1, 0.5)
    ("MXKPL", 7.0, 23.0), # maximum plant conductivity (7, 30)
    #("MXRTLN", 2000, 4000), # maximum root length (2000, 4000)
    ("VXYLEM_mm", 5.0, 75.0), # xylem volume (5, 80)
    ("DISPERSIVITY_mm", 38.0, 48.0), # dispersivity coefficient (30, 50)
    ("MAXROOTDEPTH", -2.0, -0.9), # max rooting depth (-2, -0.8)
    ("BETAROOT", 0.9, 0.94) # beta root coefficient (0.9, 0.999)
];
...
@everywhere function run_calibration(i)
    ...
    # isotopes
    iso_soil_met = obj_fun_soil_iso(z_soil_iso, obs_soil_iso_irr);
    iso_xy_rmse = obj_fun_xy_iso(z_xy_iso, obs_xy_iso_irr);

    iso_met = [iso_soil_met..., iso_xy_rmse];

    # transpiration
    sap_comp = sap_combine(sim, obs_sap_irr);
...
CSV.write("LWFBcal_output/metrics_irr_" * curDate * ".csv", metrics);
