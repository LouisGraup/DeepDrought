...
param = [ # store unmodified parameter ranges here for posterity
    # hydro parameters
    ("DRAIN", 0.2, 0.5), # drainage (0, 1)
    ("INFEXP", 0.45, 0.75), # infiltration exponent (0, 0.9)
    ("IDEPTH_m", 0.75, 1.0), # infiltration depth (m) (0.05, 1.0)
    # meteo parameters
    #("ALB", 0.15, 0.3), # surface albedo (0.1, 0.3)
    #("ALBSN", 0.4, 0.8), # snow surface albedo (0.4, 0.8)
    # soil parameters
    ("RSSA", 200, 700), # soil resistance (20, 1000)
    ("ths1", 0.53, 0.6), # theta_sat (0.15, 0.6)
    ("thr1", 0.0, 0.03), # theta_res (0.0, 0.1)
    ("ksat1", -0.2, 0.4), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("alpha1", 0.9, 1.3), # multiplier on alpha (0.5, 1.5)
    ("npar1", 1.20, 1.29), # n (1.15, 1.3)
    ("ths2", 0.17, 0.22), # theta_sat (0.15, 0.6)
    ("thr2", 0.01, 0.04), # theta_res (0.0, 0.1)
    ("ksat2", -0.4, 0.0), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("alpha2", 0.8, 1.1), # multiplier on alpha (0.5, 1.5)
    ("npar2", 1.18, 1.275), # n (1.15, 1.3)
    ("ths3", 0.185, 0.21), # theta_sat (0.15, 0.6)
    ("thr3", 0.0, 0.02), # theta_res (0.0, 0.1)
    ("ksat3", 0.15, 0.5), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("alpha3", 0.8, 1.1), # multiplier on alpha (0.5, 1.5)
    ("npar3", 1.23, 1.28), # n (1.15, 1.3)
    # plant parameters
    #("CINTRL", 0.1, 0.75), # interception storage capacity per unit LAI (0.05, 0.75)
    ("FRINTLAI", 0.16, 0.18), # interception catch fraction per unit LAI (0.02, 0.2)
    ("GLMAX", 0.007, 0.01), # stomatal conductance (0.001, 0.02)
    ("CVPD", 1.6, 2.5), # vpd sensitivity (0.5, 3)
    ("R5", 90, 140), # radiation sensitivity (50, 200)
    ...
    ("FXYLEM", 0.3, 0.45), # aboveground xylem fraction (0.1, 0.5)
    ("MXKPL", 12.0, 15.0), # maximum plant conductivity (7, 30)
    ...
    ("MAXROOTDEPTH", -2.0, -1.25), # max rooting depth (-2, -0.8)
    ("BETAROOT", 0.91, 0.95) # beta root coefficient (0.9, 0.999)
];
