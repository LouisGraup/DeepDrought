...
param = [ # store unmodified parameter ranges here for posterity
    # hydro parameters
    ("DRAIN", 0.2, 0.35), # drainage (0, 1)
    ("INFEXP", 0.5, 0.6), # infiltration exponent (0, 0.9)
    ...
    # soil parameters
    ("RSSA", 200, 600), # soil resistance (20, 1000)
    ("ths1", 0.56, 0.6), # theta_sat (0.15, 0.6)
    ("thr1", 0.0, 0.025), # theta_res (0.0, 0.1)
    ("ksat1", -0.2, 0.4), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("alpha1", 0.9, 1.1), # multiplier on alpha (0.5, 1.5)
    ("npar1", 1.23, 1.28), # n (1.15, 1.3)
    ("ths2", 0.19, 0.21), # theta_sat (0.15, 0.6)
    ("thr2", 0.027, 0.04), # theta_res (0.0, 0.1)
    ("ksat2", -0.15, 0.0), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("alpha2", 0.8, 1.0), # multiplier on alpha (0.5, 1.5)
    ("npar2", 1.22, 1.27), # n (1.15, 1.3)
    ("ths3", 0.12, 0.21), # theta_sat (0.15, 0.6)
    ("thr3", 0.0, 0.01), # theta_res (0.0, 0.1)
    ("ksat3", 0.2, 0.42), # additive factor on log10(k_sat) (-0.5, 0.5)
    ("alpha3", 0.8, 0.98), # multiplier on alpha (0.5, 1.5)
    ("npar3", 1.24, 1.265), # n (1.15, 1.3)
    # plant parameters
    #("CINTRL", 0.1, 0.75), # interception storage capacity per unit LAI (0.05, 0.75)
    ("FRINTLAI", 0.17, 0.18), # interception catch fraction per unit LAI (0.02, 0.2)
    ("GLMAX", 0.0082, 0.0095), # stomatal conductance (0.001, 0.02)
    ("CVPD", 2.0, 2.5), # vpd sensitivity (0.5, 3)
    ("R5", 98, 112), # radiation sensitivity (50, 200)
    #("T1", 6, 12), # low temperature threshold (5, 15)
    #("T2", 20, 35), # high temperature threshold (20, 35)
    ("PSICR", -1.2, -1.0), # critical water potential (-3, -1)
    ("FXYLEM", 0.3, 0.4), # aboveground xylem fraction (0.1, 0.5)
    ("MXKPL", 13.0, 15.0), # maximum plant conductivity (7, 30)
    ...
    ("MAXROOTDEPTH", -1.9, -1.4), # max rooting depth (-2, -0.8)
    ("BETAROOT", 0.935, 0.95) # beta root coefficient (0.9, 0.999)
];
