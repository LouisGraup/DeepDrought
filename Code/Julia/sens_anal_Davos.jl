...
function met_comb!(met)
    met.swc_cor_com = sqrt.((met.swc_cor15 .^ 2 + met.swc_cor50 .^ 2 + met.swc_cor80 .^ 2) / 3);
    met.et_com = sqrt.((met.et_cor .^ 2 + met.et_nse .^ 2) / 2);
    met.met_com = sqrt.((met.swc_cor_com .^ 2 + met.et_com .^ 2) / 2);
    return nothing
end
...
# calibration results
met = CSV.read("LWFBcal_output/metrics_davos_20260910.csv", DataFrame);
par = CSV.read("LWFBcal_output/param_davos_20260910.csv", DataFrame);
