...
param = [
    # plant parameters
    ("CAPACITANCE", 6.0, 10.0), # capacitance (0.1, 10)
    ("STORAGEK", 8.0, 10.0), # storage conductance (0.1, 10)
    ("VSTORAGE", 1.0, 2.5), # stem storage volume (1, 20)
...
CSV.write("LWFBcal_output/param_pfyn_ctr_cap_" * curDate * ".csv", param_out);
...
@everywhere function run_calibration(i)
    ...
    if root_params
        ...
        model = loadSPAC(cal_dir, output_prefix, simulate_isotopes = false,
        root_distribution = (beta = 0.970584, z_rootMax_m=-1.86983));
    end
...
CSV.write("LWFBcal_output/metrics_pfyn_ctr_cap_" * curDate * ".csv", metrics);
